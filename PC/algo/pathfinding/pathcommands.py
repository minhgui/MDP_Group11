    ...
    for node in path:
        ...
        if node.prevAction == (Gear.FORWARD, Steering.LEFT):
            ...
            if LF >= 1:
                ...
                command.append(f"FL{int(LF):03d}")
                command.append(f"FS{int(dis):03d}")
                LF = 0
                dis = 0
        ...
        if node.prevAction == (Gear.FORWARD, Steering.STRAIGHT):
            ...
            if SF >= 1:
                SF *= L
                command.append(f"FS{int(SF):03d}")
                SF = 0
                dis = 0
        ...
        if node.prevAction == (Gear.FORWARD, Steering.RIGHT):
            ...
            if RF >= 1:
                ...
                command.append(f"FR{int(RF):03d}")
                command.append(f"FS{int(dis):03d}")
                RF = 0
                dis = 0
        ...
        if node.prevAction == (Gear.REVERSE, Steering.LEFT):
            ...
            if LB >= 1:
                ...
                command.append(f"BL{int(LB):03d}")
                command.append(f"BS{int(dis):03d}")
                LB = 0
                dis = 0
        ...
        if node.prevAction == (Gear.REVERSE, Steering.STRAIGHT):
            ...
            if SB >= 1:
                SB *= L
                command.append(f"BS{int(SB):03d}")
                SB = 0
                dis = 0
        ...
        if node.prevAction == (Gear.REVERSE, Steering.RIGHT):
            ...
            if RB >= 1:
                ...
                command.append(f"BR{int(RB):03d}")
                command.append(f"BS{int(dis):03d}")
                RB = 0
                dis = 0
        prev = node
    ...
    for node in path[1:]:
        ...
        if gear == prevGear and steering == prevSteering:
            ...
            if prevSteering == Steering.STRAIGHT:
                commands.append("{}S{:03d}".format( #updated the command here
                    "F" if prevGear == Gear.FORWARD else "B",
                    int(sameCommandCount*unitDist)))
            else:
                commands.append("{}{}{:03d}".format(
                    "F" if prevGear == Gear.FORWARD else "B",
                    "L" if prevSteering == Steering.LEFT else "R",
                    int(sameCommandCount*unitAngle)))

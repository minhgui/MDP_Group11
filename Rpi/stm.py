...
class STMInterface:
                        ...
                        for idx, command in enumerate(commands):
                            ...
                            if message["data"]["commands"] != "SECONDLEFT" or message["data"]["commands"] != "SECONDRIGHT":
                                if idx >= len(commands) - NUM_IMAGES:
                                    ...
                                    capture_and_send_image_thread.start()
                            #if command[0] != "S":
                            elif self.resetNo % 3 == 2:
                                print("Writing reset to STM")
                                self.write_to_stm("RESET")
                            
                            print("[RPI] Writing to STM:", command)
                            self.write_to_stm(command)
                            self.resetNo = self.resetNo + 1
    ...
    def adjust_commands(self, commands):
        ...
        def combine_straight_commands(straight_commands):
            ...
            if total > 0:
                return "FS%03d" % abs(total)
            elif total < 0:
                return "BS%03d" % abs(total)
            else:
                return None
    ...
    def get_commands_to_carpark(self):
        ...
        movement_list.append(f"FS{y_adjustment:03d}")
        if self.second_arrow == 'R':
            movement_list.append("FL090")
            if x_adjustment > 0:
                movement_list.append(f"FS{x_adjustment:03d}")
            else:
                movement_list.append(f"BS{abs(x_adjustment):03d}")
